# removes the inventory.ini file first
#!/bin/bash
set -e
...
#   "k3s-server ansible_host=\($server_ip) ansible_user=ubuntu ansible_ssh_private_key_file=~/.ssh/eg223ps-keypair.pem\n\n" +
#   "[k3s_workers]\n" +
#   ([.workers[] | "k3s-worker ansible_host=\(.private_ip) ansible_user=ubuntu ansible_ssh_private_key_file=~/.ssh/eg223ps-keypair.pem ansible_ssh_common_args='\''-o ProxyJump=ubuntu@\($server_ip) -o StrictHostKeyChecking=no'\''"] | join("\n")) +
#   "\n\n[all:vars]\nansible_ssh_common_args='\''-o StrictHostKeyChecking=no'\''"
